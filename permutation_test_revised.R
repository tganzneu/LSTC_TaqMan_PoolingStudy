...
stat_meds <- c()
for (i in (1:3)){
  stat_means[i] = abs((a_means[i]) - (a_means[i+3]))
  stat_meds[i] = abs((a_meds[i]) - (a_meds[i+3]))
}
...
a_o_p_t <- perm_stat(a_o_p, 100)
a_n_p_t <- perm_stat(a_n_p, 100)
a_s_p_t <- perm_stat(a_s_p, 100)
...
stat_meds <- c()
for (i in (1:3)){
  stat_means[i] = abs((s_means[i]) - (s_means[i+3]))
  stat_meds[i] = abs((s_meds[i]) - (s_meds[i+3]))
}
stat_means <- unlist(stat_means)
stat_meds <- unlist(stat_meds)
...
# Permute for each gene for number of cycles
s_o_p <- permute(s_o, 100)
s_n_p <- permute(s_n, 100)
s_s_p <- permute(s_s, 100)
# Generate test statistics from permuted data
s_o_p_t <- perm_stat(s_o_p, 100)
s_n_p_t <- perm_stat(s_n_p, 100)
s_s_p_t <- perm_stat(s_s_p, 100)
# Calculate p-values for both means and medians
